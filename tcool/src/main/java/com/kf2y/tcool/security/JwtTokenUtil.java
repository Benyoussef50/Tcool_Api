...
public class JwtTokenUtil implements Serializable{
	...
	public Boolean canTokenBeRefreshed(final String token, final Date lastPassworsReset) {
		final Date created = getIssuedAtDateFromToken(token);
		return !isCreatedBeforeLastPasswordReset(created, lastPassworsReset) && (!isTokenExpired(token) || ignoreTokenExpiration(token));
	}
